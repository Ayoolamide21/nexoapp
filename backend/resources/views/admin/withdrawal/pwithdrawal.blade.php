                    ...
                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mt-24">
                        <div>
                            <span class="text-sm text-muted">
                                Showing {{ $data['withdrawals']->firstItem() ?? 0 }} to
                                {{ $data['withdrawals']->lastItem() ?? 0 }} of {{ $data['withdrawals']->total() }} entries
                            </span>
                        </div>
                        ...
                        <ul class="pagination d-flex flex-wrap align-items-center gap-2 justify-content-center">
                            ...
                            @if ($data['withdrawals']->onFirstPage())
                                <li class="page-item disabled">
                                    <a class="page-link bg-neutral-200 text-muted fw-semibold radius-8 border-0 d-flex align-items-center justify-content-center h-32-px w-32-px text-md"
                                        href="#">
                            ...
                            @else
                                <li class="page-item">
                                    <a class="page-link bg-neutral-200 text-secondary-light fw-semibold radius-8 border-0 d-flex align-items-center justify-content-center h-32-px w-32-px text-md"
                                        href="{{ $data['withdrawals']->previousPageUrl() }}" rel="prev">
                                        <iconify-icon icon="ep:d-arrow-left"></iconify-icon>
                                    </a>
                                </li>
                            @endif
                            ...
                            @foreach ($data['withdrawals']->links()->elements[0] as $page => $url)
                                @if ($page == $data['withdrawals']->currentPage())
                                    <li class="page-item active">
                                        <a class="page-link text-white fw-semibold radius-8 border-0 d-flex align-items-center justify-content-center h-32-px w-32-px text-md bg-primary-600"
                                            href="#">{{ $page }}</a>
                                    </li>
                                @else
                                    <li class="page-item">
                                        <a class="page-link text-secondary-light fw-semibold radius-8 border-0 d-flex align-items-center justify-content-center h-32-px w-32-px text-md bg-neutral-200"
                                            href="{{ $url }}">{{ $page }}</a>
                                    </li>
                                @endif
                            @endforeach
                            ...
                            @if ($data['withdrawals']->hasMorePages())
                                <li class="page-item">
                                    <a class="page-link bg-neutral-200 text-secondary-light fw-semibold radius-8 border-0 d-flex align-items-center justify-content-center h-32-px w-32-px text-md"
                                        href="{{ $data['withdrawals']->nextPageUrl() }}" rel="next">
